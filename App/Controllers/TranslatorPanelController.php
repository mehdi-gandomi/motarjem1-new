...
class TranslatorPanelController extends Controller
{
    ...
    public function set_order_as_completed($req,$res,$args)
    {
        $body=$req->getParsedBody();
        $result=Order::set_as_completed($body);
        if ($result){
            return $res->withJson(['status'=>true]);
        }
        return $res->withJson(['status'=>false]);
    }
    public function upload_completed_order_files($req,$res,$args)
    {
        $uploadedFiles = $req->getUploadedFiles();
        $uploadedFile = $uploadedFiles['file'];
        $directory = dirname(dirname(__DIR__)) . '/public/uploads/order/completed';
        if ($uploadedFile->getError() === UPLOAD_ERR_OK) {
            try {
                $filename = $this->moveUploadedFile($directory, $uploadedFile);
                $res->write($filename);
